    ...
    public function validateBill($attribute, $params, $validator)
    {
        ...
        if ($this->is_all == 0) {
            if (empty($this->channel_id)) {
                $this->addError('channel_name', 'Channel Name cannot bu null');
            } else {
...
//        die();
        ...
        if ($this->type == 1) { //campaign pending
            ...
            if (empty($this->adv_name)) {
                $this->addError('adv_name', 'ADV Name cannot bu null');
            } else {
                if (empty(Advertiser::getOneByUsername($this->adv_name))) {
    ...
    public function checkPendingTime($campaign_id, $bill_type, $bill_value)
    {
        ...
        if ($bill_type == 1) {
            ...
            $adv_pending = $this->getChannelPending($bill_value, $campaign_id);
