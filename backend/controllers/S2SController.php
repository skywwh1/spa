...
use Yii;
use common\models\S2s;
use common\models\S2sSearch;
use yii\web\Controller;
use yii\web\NotFoundHttpException;
use yii\filters\VerbFilter;

/**
 * S2sController implements the CRUD actions for S2s model.
 */
class S2sController extends Controller
{
    ...
    /**
     * Lists all S2s models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new S2sSearch();
    ...
    /**
     * Finds the S2s model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $campaign_id
     * @param integer $channel_id
     * @return S2s the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($campaign_id, $channel_id)
    {
        if (($model = S2s::findOne(['campaign_id' => $campaign_id, 'channel_id' => $channel_id])) !== null) {
            return $model;
        } else {
